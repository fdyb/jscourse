...
function move() {
    ...
    if (!isSnakeUnit(new_unit) && !isRockUnit(new_unit)) {
...
function haveFood(unit) {
    ...
    if (unit_classes.includes('food-unit')) {
        ...
        createFood();
        score++;
        document.querySelector('#score-info').innerText = score;
...
function createRock() {
    ...
    while (!rockCreated) { //пока препятствие не создали
        ...
        // проверка на змейку
        if ((!rock_cell_classes.includes('snake-unit')) && (!rock_cell_classes.includes('rock-unit'))) {
            var classes = '';
